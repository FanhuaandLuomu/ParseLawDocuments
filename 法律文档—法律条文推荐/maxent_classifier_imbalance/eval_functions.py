...
def get_PRF_subAcc(all_case_labels):
	p_list=[]
	r_list=[]
	f_list=[]
	total_acc=0

	for i in range(len(all_case_labels)):
		if all_case_labels[i][0]==all_case_labels[i][1]:
			# print i
			# print all_case_labels[i][0]
			# print all_case_labels[i][1]
			# print '-'*20
			total_acc+=1
		p,r,f,acc=createPRF_me(all_case_labels[i][0],all_case_labels[i][1])
		p_list.append(p[1])
		r_list.append(r[1])
		f_list.append(f[1])
		# if i==0:
	...
	subAcc=total_acc/len(all_case_labels)
	result={}
	result['ave_p']=ave_p
	result['ave_r']=ave_r
	result['ave_f']=ave_f
...
def get_hloss(all_case_labels):  # 计算测试集的hloss
	hloss_list=[]
	for i in range(len(all_case_labels)):
		hloss_list.append(get_per_hloss(\
							all_case_labels[i][0],all_case_labels[i][1]))
	hloss=sum(hloss_list)/len(hloss_list)
	return hloss

# 精确率(Accuracy)统计正确预测的标签占预测标签与真实标签集合的比例，
def get_Accuracy(all_case_labels):
	acc_list=[]
	for i in range(len(all_case_labels)):
		list1,list2=all_case_labels[i][0],all_case_labels[i][1]
		jiaoji=0
		bingji=0
...
def xiugai_pred(all_case_labels):
	for i in range(len(all_case_labels)):
		if sum(all_case_labels[i][1])==0:
			all_case_labels[i][1]=[1]+[0]*49
...
def main():
	...
	all_case_labels=cPickle.load(open('all_case_labels.pkl'))
	# all_case_labels=xiugai_pred(all_case_labels)

	print all_case_labels[0][0]
	print all_case_labels[0][1]

	result=get_PRF_subAcc(all_case_labels)  # P R F  subAcc
	print 'ave_p:',result['ave_p']
	print 'ave_r:',result['ave_r']
	print 'ave_f:',result['ave_f']
	print 'subAcc:',result['subAcc']

	hloss=get_hloss(all_case_labels)   # hloss
	print 'hloss:',hloss

	# print all_case_labels[0][0]
	# print all_case_labels[0][1]
	Accuracy=get_Accuracy(all_case_labels)  #  Accuracy
