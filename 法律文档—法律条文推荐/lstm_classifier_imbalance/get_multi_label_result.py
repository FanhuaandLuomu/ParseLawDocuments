...
def get_all_case_labels(source_path,k):  # k用于限制预测标签的个数 只保留预测概率最大的k个
	filesnames=['result_%d.txt' %(i+1) for i in range(50)]  # 结果文件列表
	print 'len(filesnames):',len(filesnames)

	all_case_laebls=[]
	for i in range(2000):  # 共2000个测试案件
		r_label=[0]*50   # 真实label    50个法律条文
		p_label=[0]*50   # 预测label
		p_prob=[0.0]*50  # 预测概率
	...
	if k>0:   # k>0时才执行预测标签限制操作
		new_all_case_labels=cut_pred_label(all_case_laebls,k)
	else:
...
def cut_pred_label(all_case_laebls,k):
	new_all_case_labels=[]
